...
const fmtCurrency = (n) => {
  if (n == null) return "₹0";
  const num = Number(n);
  if (Number.isNaN(num)) return "Rs0";
  return `Rs${num}`;
};
      ...
      <div className={dashboardStyles.contentContainer}>
        ...
        <div className={dashboardStyles.statsGrid}>
          {stats.map((stat) => {
            ...
            return (
              <div key={stat.title} className={dashboardStyles.statCard}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className={dashboardStyles.statIconContainer(stat.color)}>
                    {Icon && <Icon className={dashboardStyles.statIcon} />}
                  </div>
                </div>
              </div>
            );
          ...
          <div className={dashboardStyles.tableContainer}>
            <table className={dashboardStyles.table}>
              ...
              <tbody className={dashboardStyles.tableBody}>
                {filteredCourses.map((course, index) => (
                  <tr key={course.id || `${index}`} className={dashboardStyles.tableRow}>
                    ...
                    <td className={dashboardStyles.studentsCell}>
                      <span className={dashboardStyles.studentsText}>{course.students}</span>
                    </td>
                    ...
                    <td className="px-4 sm:px-6 py-3 sm:py-4">
                      <div className={dashboardStyles.purchasesContainer}>
                        ...
                        <span className={dashboardStyles.purchasesText}>{course.purchases}</span>
                      </div>
                    </td>
            ...
            {filteredCourses.length === 0 && !loading && (
              <div className={dashboardStyles.emptyState}>
                ...
                <p className={dashboardStyles.emptyText}>No courses found.</p>
                <button
                  type="button"
            ...
            {loading && (
              <div className={dashboardStyles.emptyState}>
                <p className={dashboardStyles.emptyText}>Loading dashboard data...</p>
              </div>
            )}
          </div>
        </div>
